...
def articulated_slam(debug_inp=True):
    ...
    #with open('expt_noise.csv','wb') as csvfile:
    #    sr = csv.writer(csvfile,delimiter=',')
    ...
    for fidx,(ids,rob_state_and_input, ldmks, ldmk_robot_obs) in enumerate(rob_obs_iter[1:]):    
        ...
        for id, ldmk_rob_obv in zip(ids,np.dstack(ldmk_robot_obs)[0]):
            ...
            #nnoise = np.random.normal(mu,sigma,3)
            #sr.writerow(nnoise)
            ...
            if ldmk_am[id] is None:
                motion_class.process_inp_data([0,0], slam_state,ldmk_rob_obv,init_pt[id])
            ...
            if id == 40:
                ...
                prob_plot1.append(tmp)
            if id == 41:
                tmp = motion_class.prior.copy()
                prob_plot2.append(tmp)
            if id == 13:
                tmp = motion_class.prior.copy()
